...
class FuzzyClass:
    ...
    def resetInitData(self):
        """
        Reset initial GPS data received from sensors and compass initial angle.
        """
        # None the GPS data.
        self.init_time_string = None
        self.init_date_string = None
    ...
    def runStatic(self):
        ...
        with open('staticController.csv', 'a', newline='') as csvfile:
            ...
            bearing += 180
            angle_diff = self.calc_angle(angle, bearing)
            left = 0
            right = 0
            ...
            if self.latitude is not None and distance > 1.5:
                ...
                if abs(angle_diff) < 90:
                    ...
                    self.l_motor.ChangeDutyCycle(left*20)
                    self.r_motor.ChangeDutyCycle(right*20*0.5)
                # Backward movement.
                else:
                    print('c')
                    left, right = self.boat_controller.fuzzy_controller(1.0, 0,  angle_diff/180.0, self.norm_left_motor_speed, self.norm_right_motor_speed)
                    ...
                    self.l_motor.ChangeDutyCycle(right*20)
                    self.r_motor.ChangeDutyCycle(left*20*0.5)
            # Nothing happen.
            else:
                # Write speed to boat.
                print('a')
