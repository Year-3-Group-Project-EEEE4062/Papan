    ...
    try:
        with open('boat.csv', 'w', newline='') as csvfile:
            ...
            while True:
                ...
                right = 0
                # First control.
                if latitude != None and abs(angle_diff) > 45 and distance > 2.5:
                    # First part.
                    left, right = boat_controller.fuzzy_controller(1.0, 0, angle_diff / 180.0, norm_left_motor_speed, norm_right_motor_speed)
                    # Write speed to boat.
                    GPIO.output(20, GPIO.HIGH)
                    GPIO.output(21, GPIO.LOW)
                    l_motor.ChangeDutyCycle(left*20)
                    r_motor.ChangeDutyCycle(right*20)
                    
                    
                # Second control.
                elif latitude != None and abs(angle_diff) > 135 and distance > 2.5:
                    left, right = boat_controller.fuzzy_controller(1.0, 0,  angle_diff/180.0, norm_left_motor_speed, norm_right_motor_speed)
                    # Write speed to boat.
                    GPIO.output(20, GPIO.LOW)
                    GPIO.output(21, GPIO.HIGH)
                    l_motor.ChangeDutyCycle(right*20)
                    r_motor.ChangeDutyCycle(left*20)
