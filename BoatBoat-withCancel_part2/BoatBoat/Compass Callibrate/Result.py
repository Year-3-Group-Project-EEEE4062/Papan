...
class MechaQMC5883:
    ...
    def calibrate(self, num_samples=300):
        ...
        # Method 2 for callibration.
        x_sf = max(y_samples) - min(y_samples)/max(x_samples) - min(x_samples)
        y_sf = max(x_samples) - min(x_samples)/max(y_samples) - min(y_samples)
        x_offset = ((max(x_samples) - min(x_samples)/2) - max(x_samples)) * x_sf
        y_offset = ((max(y_samples) - min(y_samples)/2) - max(y_samples)) * y_sf
        print(f"Calibration complete. Scaling Factor X: {x_sf}, Scaling Factor Y: {y_sf}, Offset X: {x_offset}, Offset Y: {y_offset}")
        ...
        # Plot hard magnetic calibrated data.
        plt.subplot(1, 3, 2)
        plt.scatter(x_samples-self.offset_x, y_samples-self.offset_y, c='r', label='Calibrated Data 1')
        plt.xlabel('Calibrated X')
        plt.ylabel('Calibrated Y')
        plt.title('After Hard Magnetic Calibration')
        plt.legend()

        # Plot hard magnetic calibrated data.
        plt.subplot(1, 3, 3)
        plt.scatter(x_sf*x_samples+x_offset, y_sf*y_samples+y_offset, c='r', label='Calibrated Data 2')
        plt.xlabel('Calibrated X')
        plt.ylabel('Calibrated Y')
        plt.title('After Calibration using Algorithm 2')
        plt.legend()
