...
class autoClass:
    ...
    def sendBoatLocationToMedium(self):
        ...
        encodedMssg = self.encoder.packFloats(1, 0, loc)
    ...
    def processAutoMssg(self, autoMssg, boatBusy, quitTemperature, takeReading):
        ...
        if (autoMssg[0] == 0):
            ...
            encodedMssg = self.encoder.packIntegers(1, 4, [0])
            self.send_cb(encodedMssg)
        ...
        # User done taking new waypoints.
            ...
            if expectedWaypoints == self.obtainedWaypoints:
                ...
                encodedMssg = self.encoder.packIntegers(1, 2, [0])
                self.send_cb(encodedMssg)
                ...
                encodedMssg = self.encoder.packIntegers(1, 3, [0])
                self.send_cb(encodedMssg)
...
#             print("--------------------------")
            ...
            if distance_left<100 or distance_left<100 or distance_left<100:
                ########################################################################
                ...
                left, right = self.auto.auto_controller(distance_left/100.0, distance_front/100.0, distance_right/100.0, self.auto_norm_left_motor_speed, self.auto_norm_right_motor_speed)
                ...
                # Maybe also include here sending back a message back to the phone.
