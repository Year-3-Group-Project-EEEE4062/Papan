...
class encoderMssg:
    ...
    def packFloats(self , mode, mssgType, mssg):
        """
        Function to pack message of type double to list of uint8.

        Args:
            mode: either remote (int 1) or auto (int 0)
        ...
        encoded.extend(struct.pack('i', mssgType)) # what kind of auto message is it?
        encoded.extend(struct.pack('f' * len(mssg), *mssg)) # the data itself

        return encoded
        
    def packIntegers(self , mode,  mssgType, mssg):
        """
